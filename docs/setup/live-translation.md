...
**Production behind a reverse proxy:** captions and live status use SSE (`/api/translation/public/{slug}/events`). If listeners see long silence then a huge paragraph dump (or a stuck “not connected” banner until refresh), the proxy is buffering the stream. Current builds send `X-Accel-Buffering: no` so nginx/NPM usually need no custom Advanced config — see [deployment-guide.md → Reverse Proxy Checklist](../deployment-guide.md#reverse-proxy-checklist).
