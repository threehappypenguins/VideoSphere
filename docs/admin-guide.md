...
Roles are typically stored as a field on the user document:
...
4. For `/admin/*`, calls `/api/auth/session-role` once (session check + `user_profiles.role` via the server repository; middleware stays fetch-only in `proxy.ts`)
