...
/**
 * Searches SermonAudio series by title for the authenticated user's broadcaster.
 * Proxies `GET /v2/node/broadcasters/{id}/series?searchKeyword=…`.
 * @param req - Incoming GET request with `q` query parameter.
 * @returns JSON list of matching series options, or a structured error.
 */
export async function GET(req: NextRequest) {
  ...
  if (query.length < SERMON_AUDIO_SERIES_SEARCH_MIN_LENGTH) {
    const errRes: ApiError = {
      error: 'Bad Request',
      message: `Search query must be at least ${SERMON_AUDIO_SERIES_SEARCH_MIN_LENGTH} characters`,
      statusCode: 400,
    };
  ...
  try {
    const series = await searchSermonAudioSeries(
      connection.apiKey,
